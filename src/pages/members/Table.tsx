    ...
    <div className="w-full bg-dongker">
     ...
     <div className='flex flex-row gap-3 rounded-full border-gray-500 bg-gray-500/20 border-[1px]  pl-4 py-1 items-center min-w-[400px] self-end mb-10 text-teks'>
      ...
      <input type="text" 
        ...
        value={searchInput}
        onChange={(e) => setSearchInput(e.target.value)} className='focus:outline-none bg-transparent text-teks text-sm' />
      </div>
     </div>
      <Card className="relative overflow-scroll w-full items-center flex justify-center bg-dongker">
        ...
        <table className="w-full table-auto text-left">
          ...
          <tr>
            {TABLE_HEAD.map((head) => (
              <th key={head} className="border-b border-white/40 py-4 text-teks">
                <Typography
                  variant="small"
          ...
          <tbody>
            {filteredData.map(({ nama, contact, email }, index) => {
              ...
              const classes = isLast ? "py-4" : "py-4  border-b border-white/20";
              const url = 'dpd/indonesia-'+nama.toLowerCase().replace(/\s+/g, "")
              return (
                <tr key={index}>
                  <td className={classes}>
                    <Typography variant="small" color="blue-gray" className="font-normal ">
                      <a href={url} className="text-subteks">{nama}</a>
                    </Typography>
                  </td>
                  
                  <td className={classes}>
                    <Typography variant="small" color="blue-gray" className="font-normal text-subteks">
                      {contact}
                    </Typography>
                  </td>
                  <td className={classes}>
