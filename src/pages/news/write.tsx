...
export default function Write() {
  ...
  async function save(e:any){
    ...
    if (editorRef.current) {
      ...
      const result = await fetch('/api/addnews', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='relative w-full h-screen'>
      ...
      <div className="bg-white rounded-xl flex flex-col gap-10 p-10 w-[450px] border-2 overflow-y-auto">
        ...
        <input type="text" value={value} onChange={e => setValue(e.target.value)} placeholder='Masukkan judul berita' name='title' id='title' className=' focus:border-black rounded-lg  border focus:outline-none p-2' />
        </div>
