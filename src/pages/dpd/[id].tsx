...
const DetailDPD = () => {
  ...
  return (
   ...
   <WithNavbar></WithNavbar>
        ...
        <div className={`bg-dongker text-teks ${!isDesktop ? 'w-[800px]' : 'w-full'}`}>
        <div className="p-10">
        {
          ...
          filtered.length > 0 ? 
            ...
            <div key={item.nama}>
              ...
              <h1 className='pt-20 text-4xl  text-center font-extrabold text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-slate-400 '>DPD {item.nama}</h1>
   
          
              <div className='flex flex-col bg-gradient-to-b mt-10 from-transparent to-purple-950 rounded-b-3xl gap-4  text-teks p-6 border-t-0 '>
             <Map targetState={stateName}></Map>
             </div>
              <div className="flex flex-row pt-10 gap-10">
              
              <iframe className="rounded-xl" src={item.gmaps} ></iframe>
              ...
              <Link target="_blank" className="flex mb-10 items-center justify-center flex-row gap-4 w-fit text-center p-6 py-2 rounded-xl bg-gradient-to-br from-purple-950 to-transparent border-[1px] border-white/20 text-white" href={`https://api.whatsapp.com/send?phone=62${parseFloat(item.contact.split(',')[0])}`}>
